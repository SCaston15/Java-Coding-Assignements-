...
public void bubbleSort() {
	...
	for(int i=0;i<length();i++){
	 for(int j=0;j<length()-(i+1);j++){
	  if((Integer)listArray[j]>(Integer)listArray[j+1]){
	   E temp = listArray[j];
...
/*returns the number of comparisons needed to find given element*/
...
while(low+1!=high){
	int mid = ((low+high)/2);
	if((Integer)listArray[mid] > (Integer)key) {
	 high = mid;
	 compares++;
	}
	
	else if((Integer)listArray[mid] < (Integer)key){
	 low = mid;
	 compares++;
	}

	else return compares;
}
return compares;
}
}
